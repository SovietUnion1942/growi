...
- [x] 5. Core: 書きかけ(WIP)ページウィジェット
- [x] 5.1 自分の書きかけ(WIP)ページを取得するクライアント側の仕組みを追加する
  - タスク2.1のエンドポイントを呼び出し、結果をキャッシュ・再利用できる形で提供する
  - 完了状態: ホーム画面のコンポーネントから自分の WIP ページ一覧を取得できることを確認できる
  - _Requirements: 4.2_
  - _Depends: 2.1_
- [x] 5.2 書きかけ(WIP)ページウィジェットを実装する
...
- [x] 6. Integration: ホーム画面への統合
- [x] 6.1 ウィジェットエリアのコンテナを実装する
