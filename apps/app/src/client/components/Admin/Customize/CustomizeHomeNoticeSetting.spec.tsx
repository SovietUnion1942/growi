...
describe('CustomizeHomeNoticeSetting', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('syncs the current home notice from the container into the editor', async () => {
    const container = new AdminCustomizeContainer();
    await container.setState({
      currentCustomizeHomeNotice: '## Notice\n\nhello members',
    });
  ...
  it('saves the edited home notice and reflects the saved value on submit', async () => {
    mockedApiv3Put.mockResolvedValue(
      mock<Apiv3PutResponse>({
        data: {
          customizedParams: { customizeHomeNotice: 'updated notice body' },
        },
      }),
    );
    const container = new AdminCustomizeContainer();
    await container.setState({
      currentCustomizeHomeNotice: 'old notice body',
    });
    ...
    await waitFor(() => {
      expect(mockedApiv3Put).toHaveBeenCalledWith(
        ...
        { customizeHomeNotice: 'old notice body' },
      );
    });
